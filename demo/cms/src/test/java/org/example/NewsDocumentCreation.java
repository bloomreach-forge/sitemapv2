...
public class NewsDocumentCreation {

    public static void createNewsDocuments(final Session session, int amount) throws RepositoryException, WorkflowException, RemoteException {
        WorkflowManager workflowManager = ((HippoWorkspace)session.getWorkspace()).getWorkflowManager();
        String basePath = "/content/documents/myproject/news";
    ...
    private static void createNewsDocument(WorkflowManager workflowManager, final Session session, final String basePath, final int year, final int month, final String name) throws RepositoryException, WorkflowException, RemoteException {
        ...
        if (!base.hasNode(yearString)) {
            ...
            folderWorkflow.add("new-news-folder", "hippostd:folder", yearString);
        }
        ...
        if (!yearNode.hasNode(monthString)) {
            ...
            folderWorkflow.add("new-news-folder", "hippostd:folder", monthString);
        }
        ...
        String add = folderWorkflow.add("new-news-document", "myproject:newsdocument", name);
    ...
    private static void populateRandomPropertiesOnNewsDocument(final Node documentNode) throws RepositoryException {
        ...
        documentNode.setProperty("myproject:author", RandomStringUtils.random(10, true, false));
        ...
        documentNode.setProperty("myproject:source", RandomStringUtils.random(10, true, false));
