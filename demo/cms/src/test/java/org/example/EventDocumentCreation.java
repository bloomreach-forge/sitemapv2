...
public class EventDocumentCreation {

    private static void createEventsDocument(final Session session, final String basePath, final int year, final int month, final String name) throws RepositoryException, WorkflowException, RemoteException {
        ...
        if (!base.hasNode(yearString)) {
            ...
            folderWorkflow.add("new-events-folder", "hippostd:folder", yearString);
        }
        ...
        if (!yearNode.hasNode(monthString)) {
            ...
            folderWorkflow.add("new-events-folder", "hippostd:folder", monthString);
        }
        ...
        String add = folderWorkflow.add("new-events-document", "myproject:eventsdocument", name);
        ...
        populateRandomPropertiesOnEventDocument(draft);
    }

    private static void populateRandomPropertiesOnEventDocument(final Node documentNode) throws RepositoryException {
        ...
        documentNode.setProperty("myproject:enddate", Calendar.getInstance());
        documentNode.setProperty("myproject:introduction", RandomStringUtils.random(50, true, false));
        documentNode.setProperty("myproject:location", RandomStringUtils.random(10, true, false));
    ...
    public static void createEventsDocuments(final Session session, int amount) throws RepositoryException, WorkflowException, RemoteException {
        String basePath = "/content/documents/myproject/events";
        for (int i = 0; i < amount; i++) {
            createEventsDocument(session, basePath, getRandomNumberInRange(2000, 2019), getRandomNumberInRange(1, 12), RandomStringUtils.random(10, true, false) + i);
        }
    }
}
